...
public class SearchView extends EditText {
    ...
    private void DrawSearchIcon(Canvas canvas) {
        if (this.getText().toString().length() == 0) {
            float textWidth = paint.measureText("请输入用户名或者车牌号码搜索");
            ...
            canvas.drawText("请输入用户名或者车牌号码搜索", getScrollX() + searchSize + 8, getScrollY() + (getHeight() - (getHeight() - textHeight) / 2) - paint.getFontMetrics().bottom - dy, paint);
            canvas.restore();
        }
    }
